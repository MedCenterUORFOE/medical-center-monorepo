...
export async function GET(
  request: Request,
  { params }: { params: { doctor_Id: string } } // 👈 වෙනස 1: මෙතන 'I' අකුර කැපිටල් කළා (ෆෝල්ඩරයේ නමට ගැළපෙන්න)
) {
  try {
    // --- RATE LIMITING ---
    const forwardedFor = request.headers.get('x-forwarded-for');
    ...
    // 👈 වෙනස 2: params වලින් එන doctor_Id අරගෙන අපේ doctor_id කියන නමට සමාන කළා
